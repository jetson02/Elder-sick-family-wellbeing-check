...
export class MemStorage implements IStorage {
  ...
  private initializeSampleData() {
    ...
    // Create family connections for Martha
    ...
    // Create family connections for John
    this.addFamilyConnectionInternal({
      userId: john.id,
      familyMemberId: martha.id,
      relationship: "Mother"
    });
    ...
    // Create some check-ins for Martha
    this.createCheckInInternal({
      userId: martha.id,
      mood: "good",
      message: "Had a wonderful morning. Just took my medication."
    });
    
    // Make the timestamp a bit earlier (2 hours ago)
    const twoHoursAgo = new Date();
    twoHoursAgo.setHours(twoHoursAgo.getHours() - 2);
    const marthaCheckIns = this.checkIns.get(martha.id);
    if (marthaCheckIns && marthaCheckIns.length > 0) {
      marthaCheckIns[0].timestamp = twoHoursAgo;
    }
    
    // Add initial location for Martha
    ...
    // Add initial location for Sarah
    this.createLocationInternal({
      userId: sarah.id,
      latitude: "40.7400",
      longitude: "-74.0100",
